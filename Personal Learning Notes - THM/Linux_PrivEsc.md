...
## Questions
~~~
1. run "id" command what is result? uid=1000(user) gid=1000(user) groups=1000(user),24(cdrom),25(floppy),29(audio),30(dip),44(video),46(plugdev)
~~~
...
## Questions
~~~
1. What is the root user's password hash? $6$Tb/euwmK$OXA.dwMeOAcopwBl68boTG5zi65wIHsc84OWAIye5VITLLtVlaXvRDJXET..it8r.jbrlpfZeMdwD3B0fGxJI0:17298:
2. What hashing algorithm was used to produce the root user's password hash? sha512crypt
...
## Weak File Permissions - Writable /etc/psswd
~~~
...
## Questions
~~~
1. Run the "id" command as the newroot user. What is the result? uid=0(root) gid=0(root) groups=0(root)
~~~
...
## Questions
~~~
1. How many programs is "user" allowed to run via sudo? 11
...
## Questions
~~~
1. What is the value of the PATH variable in /etc/crontab? /home/user:/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin
~~~
...
## SUID / SGID Executables - Enviroment Variables
~~~
...
## Questions
~~~
1. What is the full mysql command the user executed? mysql -h somehost.local -uroot -ppassword123
~~~
...
## Questions
~~~
1. What file did you find the root user's credentials in? /etc/openvpn/auth.txt
~~~
...
ssh -i root_key root@10.10.137.8
~~~
...
## Questions
~~~
1. What is the name of the option that disables root squashing? no_root_squash
~~~
